...
export async function POST(
  ...
  try {
    ...
    const { title, category = "Other", expenseDate, items } = body;
    ...
    // Validate and build line items with precise split amounts
...
export async function DELETE(
  ...
  // Creator or group admin can delete expense
  if (expense.createdById !== currentUserId && membership.role !== "ADMIN") {
    return NextResponse.json(
      { error: "Only the creator or admin can delete this expense" },
      { status: 403 }
    );
  }
...
export async function PUT(
  ...
  try {
    const body = await request.json();
    const { title, category = "Other", items } = body;
